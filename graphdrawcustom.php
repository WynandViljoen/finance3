<!DOCTYPE HTML>
<html>
	<head>
    </head>
    <body>
        <script src="js/jquery-1.9.1.min.js"></script>
        <script language="javascript" type="text/javascript" src="datepicker.js"></script>
		<style type="text/css" media="screen">
			@import "datepicker.css";
		</style>
<?php
	
	include("header.html");
	include("graphheading.php");

	$myseries = "";

	for ($i=$startMonth; $i <= $counter; $i++)
		{
			$year = floor($startYear + ($i / 12));
			$month = $i % 12;
			$xaxis [$i - $startMonth] = date("Y-m-t", strtotime($year . "-". $month ."-01"));
			$yearmonth [$i - $startMonth] = date("Y-m", strtotime($year . "-". $month ."-01"));
		}	


	$sql3 = "SELECT * FROM `graphs` WHERE (`AccountsGraph` = '0')";
	if ($result3=mysqli_query($con,$sql3))
	while ($row3=mysqli_fetch_row($result3))
	{
		for ($i=$startMonth; $i <= $counter; $i++)
		$expenseSeries[$i - $startMonth] = 0;
			
		$sql = "SELECT * FROM `graphelement` WHERE (`graphname` = '" . $row3['0'] ."')";
		if ($result=mysqli_query($con,$sql))
		while ($row=mysqli_fetch_row($result))
		{
			$sql2 = "SELECT `total`, `month` FROM `" . $row['2'] . "` WHERE (`" . $row['3'] . "` = '" . $row['4'] . "') GROUP BY `month` ASC";
			if ($result2=mysqli_query($con,$sql2))
				while ($row2=mysqli_fetch_row($result2))
					if (is_numeric(array_search($row2['1'],$yearmonth)))
					{
						if ($row['5'] == 0)
							$expenseSeries[array_search($row2['1'],$yearmonth)] += number_format((float)$row2['0'], 2, '.', '');
						else
							$expenseSeries[array_search($row2['1'],$yearmonth)] -= number_format((float)$row2['0'], 2, '.', '');
					}
		}

		$total = 0;
		$average = 0;

		for ($i=0;$i < count($expenseSeries); $i++)
			$total += $expenseSeries[$i];

		$average = $total / count($expenseSeries);
		$total = number_format((float)$total, 2);
		$average = number_format((float)$average, 2); 

		$myseries .= "name: '" . $row3['0'] . " ($currencysymbol $total | $currencysymbol $average)" . "',";	
		$myseries .= " data: [";
		
		for ($i=0;$i < count($expenseSeries); $i++)
			$myseries .=  $expenseSeries[$i] . ", ";

		$myseries = rtrim($myseries, ", ");
		$myseries .= "], ";
		if ($row3['2'] == 1)
			$myseries .= "visible: true";
		else 
			$myseries .= "visible: false";
		$myseries .= "}, {";
	}

/////////////////////////////////// ACCOUNTS GRAPHS ////////////////////////////////
	$sql3 = "SELECT * FROM `graphs` WHERE (`AccountsGraph` = '1')";
	if ($result3=mysqli_query($con,$sql3))
	while ($row3=mysqli_fetch_row($result3))
	{
		$Savings = 0;
		$sql = "SELECT DISTINCT `FieldString` FROM `graphelement` WHERE (`graphname` = '" . $row3['0'] . "')";
		if ($result=mysqli_query($con,$sql))
		while ($row=mysqli_fetch_row($result))
		{	
			$sql2 = "SELECT `Balance` FROM `accounts` WHERE (`Account` = '" . $row['0'] . "')";
			if ($result2=mysqli_query($con,$sql2))
			while ($row2=mysqli_fetch_row($result2))
			{
				$Savings += $row2['0'];
			}
		}

		for ($i=$startMonth; $i <= $counter; $i++)
		$currentbalance[$i - $startMonth] = $Savings;
		
		$sql = "SELECT * FROM `graphelement` WHERE (`graphname` = '" . $row3['0'] ."')";
		if ($result=mysqli_query($con,$sql))
		while ($row=mysqli_fetch_row($result))
		{	
			$operation = 1;
			if ($row['5'] == 1)
				$operation = -1;

			$sql2 = "SELECT `total`, `month` FROM `" . $row['2'] . "` WHERE (`" . $row['3'] . "` = '" . $row['4'] . "') ORDER BY `month` ASC";
			if ($result2=mysqli_query($con,$sql2))
			while ($row2=mysqli_fetch_row($result2))
			if (is_numeric(array_search($row2['1'],$yearmonth)))
			{	
				for ($i = array_search($row2['1'],$yearmonth); $i < count($yearmonth) ; $i++)
				$currentbalance[$i] += $operation * number_format((float)$row2['0'], 2, '.', '');		
			}
			else if ($row2['1'] < $yearmonth[0])
				for ($i=0;$i < count($yearmonth); $i ++)
					$currentbalance[$i] += $operation * number_format((float)$row2['0'], 2, '.', '');		
		}

		$lastvalue = $currentbalance[count($currentbalance) - 1];
		$myaverage = number_format(($lastvalue - $currentbalance[0]) / (count($currentbalance) - 1), 2);
		$lastvalue = number_format((float)$lastvalue, 2); 

		$myseries .= "name: '" . $row3['0'] . " ($currencysymbol $lastvalue | $currencysymbol $myaverage)" . "',";	
		$myseries .= " data: [";
	
		for ($i=0;$i < count($currentbalance); $i++)
		{
			$currentbalance[$i] = number_format((float)$currentbalance[$i], 2, '.', '');
			$myseries .=  $currentbalance[$i] . ", ";
		}

		
		$myseries = rtrim($myseries, ", ");
		$myseries .= "], ";
		if ($row3['2'] == 1)
			$myseries .= "visible: true";
		else 
			$myseries .= "visible: false";
		$myseries .= "}, {";
	}

	$myseries = rtrim($myseries, "}, {");
	$myXaxisString = "[";

	for ($i=0;$i < count($xaxis); $i++)
	{
		$myXaxisString .= "'" . $xaxis[$i] . "', ";
	}
	$myXaxisString = rtrim($myXaxisString, ", ");
	$myXaxisString .= "]";

?>


 <script src="js/jquery-1.9.1.min.js"></script>


<script src="https://code.highcharts.com/highcharts.js"></script>
<script src="https://code.highcharts.com/modules/series-label.js"></script>
<script src="https://code.highcharts.com/modules/exporting.js"></script>
<script src="https://code.highcharts.com/modules/export-data.js"></script>
<script src="https://code.highcharts.com/modules/accessibility.js"></script>

<script type="text/javascript">

$(function () {
	$('#container').highcharts({

    title: {
        text: 'Income/Expense Graphs (Total | Average per month) ----- Account Graphs (LastValue | Average growth per month)'
    },

    yAxis: {
        title: {
            text: 'Total for Month'
        }
    },
    xAxis: {
        categories: <?php echo $myXaxisString; ?>
    },
    

    legend: {
        layout: 'horizontal',
        align: 'left',
        verticalAlign: 'bottom'
    },

   

    series: [{
        <?php echo $myseries; ?>
    }],

    responsive: {
        rules: [{
            condition: {
                maxWidth: 900
            },
            chartOptions: {
                legend: {
                    layout: 'horizontal',
                    align: 'center',
                    verticalAlign: 'bottom'
                }
            }
        }]
    }
    });
});

</script>

<script src="https://code.highcharts.com/highcharts.js"></script>
<script src="https://code.highcharts.com/modules/data.js"></script>
<script src="https://code.highcharts.com/modules/drilldown.js"></script>
		
<figure class="highcharts-figure">
    <div id="container" style="height: 700px"></div>
</figure>

	</body>
</html>
